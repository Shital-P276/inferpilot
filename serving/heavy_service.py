"""
Heavy tier serving endpoint.

Same pattern as serving/fast_service.py, wrapping HeavyEfficientNet instead.
Exposes Prometheus metrics at /metrics.

Run directly:
    uvicorn serving.heavy_service:app --host 0.0.0.0 --port 8003 --reload
...
# ---- Config ----
CHECKPOINT_PATH = "training/checkpoints/heavy_efficientnet_best.pt"
IMG_SIZE = 224
...
app = FastAPI(title="InferPilot - Heavy Tier Service")
...
def load_model():
    ...
    model = HeavyEfficientNet(num_classes=len(CLASS_NAMES))
    model.load_state_dict(checkpoint["model_state_dict"])
    model.to(DEVICE)
    model.eval()

    print(f"[heavy_service] Loaded checkpoint from {CHECKPOINT_PATH} on {DEVICE}")
    print(f"[heavy_service] Checkpoint epoch={epoch}, val_acc={val_acc}")
    print(f"[heavy_service] Classes ({len(CLASS_NAMES)}): {CLASS_NAMES}")
