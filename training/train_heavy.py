"""
Training script for the Heavy tier (EfficientNet-B0, transfer learning).
Two-phase fine-tuning, same pattern as Balanced:
  Phase 1: backbone frozen, train classifier head only
  Phase 2: unfreeze last few blocks, fine-tune at lower LR

Run: python training/train_heavy.py
"""
import time
...
def main():
    ...
    model = HeavyEfficientNet(num_classes=NUM_CLASSES, freeze_backbone=True).to(DEVICE)
    ...
    best_val_acc = 0.0
    best_ckpt_path = CHECKPOINT_DIR / "heavy_efficientnet_best.pt"
    global_epoch = 0

    with mlflow.start_run(run_name="heavy_efficientnet"):
        mlflow.log_params({
        ...
        curve_path = CHECKPOINT_DIR / "heavy_efficientnet_curves.png"
